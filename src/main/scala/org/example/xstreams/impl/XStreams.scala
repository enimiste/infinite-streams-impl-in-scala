...
object XStreams {
   ...
   * @return
   */
  def once[T](elem: T): FiniteXStream[T] = new NoEmptyXStream[T](elem, new EmptyXStream[T])
   ...
   * @return
   */
  def iterate[T](elem: T, op: T => T): XStream[T] = new NoEmptyXStream[T](elem, iterate(op(elem), op))
