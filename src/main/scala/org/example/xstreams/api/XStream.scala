...
/** XStream public API
  *
  */
trait XStream[T] extends IterableOnce[T] {
  ...
  /** Returns a new stream starting from the first element that don't match the
    * predicate
    *
    * @param predicate
    * @return
  ...
  /** Returns a new stream where each element is transformed version of the
    * original one (applying the given mapping function)
    *
    * @param mapping
    * @tparam B
    * @return
  ...
  /** Returns a new stream that merges all the sub stream into one, after
    ...
    * @return
    */
  def flatMap[B, C](mapping: T => B)(implicit
      asIterableOne: B => IterableOnce[C]
  ): XStream[C] =
    map(mapping).flatten(asIterableOne)
  ...
  /** Returns a new stream where each element is a tuple of the elements of this
    * stream and another. The length of this new stream is the length of the
    * smallest one.
    *
    * @param other
    * @tparam B
  ...
  /** Returns a new stream where each element is a finite stream of windowSize
